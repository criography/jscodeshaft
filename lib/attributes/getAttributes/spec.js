...
const SOURCE_SPREAD_PROPS = `const Component = () => <SpreadProps {...props} />`
...
describe('attributes::getAttributes()', () => {
  ...
  describe('attributes', () => {
    ...
    describe('spread props', () => {
      it('should respect spread arguments', () => {
        const processor = (fileInfo, api) => {
          const {root} = getProcessorScaffold(fileInfo, api);

          const spreadPropsElement = root
            .findJSXElements('SpreadProps')
            .nodes()
            .at(0);

          return getAttributes(spreadPropsElement);
        };

        const ast = toAst(
          processor,
          {parser: 'flow'},
          {source: SOURCE_SPREAD_PROPS},
        );

        expect(ast).toBeArrayOfSize(1);
        expect(ast[0].type).toBe('JSXSpreadAttribute');
      });


      it('should disregard spread arguments', () => {
        const processor = (fileInfo, api) => {
          const {root} = getProcessorScaffold(fileInfo, api);
          ...
          return getAttributes(spreadPropsElement, {wantedAttributes: 'hello'});
        };
