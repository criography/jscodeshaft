...
// this
const {getSpecifierNamesFromImport} = require('../');
...
const getAst = (source, specifierType) => {
  const processor = (fileInfo, api) => {
    ...
    return getSpecifierNamesFromImport(importNode, specifierType);
  };
  ...
  describe('no or invalid nodes', () => {
    NON_NODES.forEach(node => {
      it(`should return null on: ${printType(node)}`, () => {
        const processor = () => {
          return getSpecifierNamesFromImport(node);
        };
    ...
    it('should return null on: non-import node', () => {
      const processor = (fileInfo, api) => {
        ...
        return getSpecifierNamesFromImport(nonElementNode);
      };
